defmodule LiveViewStudio.ServerssTest do
  use LiveViewStudio.DataCase

  alias LiveViewStudio.Serverss

  describe "servers" do
    alias LiveViewStudio.Serverss.Server
    ...
    test "list_servers/0 returns all servers" do
      ...
      assert Serverss.list_servers() == [server]
    end

    test "get_server!/1 returns the server with given id" do
      server = server_fixture()
      assert Serverss.get_server!(server.id) == server
    end

    test "create_server/1 with valid data creates a server" do
      assert {:ok, %Server{} = server} = Serverss.create_server(@valid_attrs)
    ...
    test "create_server/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Serverss.create_server(@invalid_attrs)
    end

    test "update_server/2 with valid data updates the server" do
      server = server_fixture()
      assert {:ok, %Server{} = server} = Serverss.update_server(server, @update_attrs)
    ...
    test "update_server/2 with invalid data returns error changeset" do
      server = server_fixture()
      assert {:error, %Ecto.Changeset{}} = Serverss.update_server(server, @invalid_attrs)
      assert server == Serverss.get_server!(server.id)
    end

    test "delete_server/1 deletes the server" do
      server = server_fixture()
      assert {:ok, %Server{}} = Serverss.delete_server(server)
      assert_raise Ecto.NoResultsError, fn -> Serverss.get_server!(server.id) end
    end

    test "change_server/1 returns a server changeset" do
      server = server_fixture()
      assert %Ecto.Changeset{} = Serverss.change_server(server)
    end
  end
end
